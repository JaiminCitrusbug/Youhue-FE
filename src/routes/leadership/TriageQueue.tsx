 ...
 *      "Escalated" status/segmented filter/"open flag" drill-in are illustrative preview chrome —
 *      the real endpoint this ticket built (`GET /risk/triage`) is deliberately a MINIMAL,
 *      render-only list of open triage-band flags (band decision + hand-off is this ticket's DoD;
 *      acknowledge/escalate/close status transitions and per-class scoping are a review-triggered
 *      future ticket's surface, not invented here). Only the fields the real API returns are
 *      rendered: student, signal (flag type), risk score, raised time. The stat tiles show a
 *      single real count (this list's length) rather than four illustrative ones.
...
import { Card, CardBody, CardHeader, EmptyState, Icon, PersonCell, StatTile, Table, Tag, theme } from "@design/components"
