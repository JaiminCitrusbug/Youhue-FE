...
import { SignInCode, SignInName, SignInQR } from "./screens"
...
export function StudentSignInApp() {
  ...
  return (
    <Routes>
      ...
      <Route
        path="scan"
        element={<SignInQR onBack={() => navigate(CODE_PATH)} onTypeCode={() => navigate(CODE_PATH)} />}
      />
      <Route
        path="who"
        element={
          <SignInName onConfirm={submit} onBack={() => navigate(CODE_PATH)} submitting={submitting} error={error} />
        }
      />
    </Routes>
  )
